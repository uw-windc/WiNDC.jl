...
function state_dissagregation_model_mcp_year(GU::GamsUniverse,year::Symbol)
    ...
    @NLexpressions(model,begin
        ...
        prf_Y[r=R,s=S],
                sum(PA[r,g]*I_PA_Y[r,g,s] for g∈G) +
                    ...
                    PL[r]*I_PL_Y[r,s] + PK[r,s]*I_PK_Y[r,s]  + R_Y_RA[r,s] - 
                         ...
                         sum(PY[r,g]*O_Y_PY[r,g,s] for g∈G)
        ...
        prf_A[r=R,g=G],
                ...
                PN[g]*I_PN_A[g,r] + PD[r,g]*I_PD_A[r,g] + PFX*I_PFX_A[r,g] + sum(PM[r,m]*I_PM_A[r,m,g] for m∈M) + R_A_RA[r,g] -
                         ...
                         (PA[r,g]*O_A_PA[r,g] + PFX * O_A_PFX[r,g])

        prf_MS[r=R,m=M],	sum(PN[gm]*I_PN_MS[gm,r,m] + PD[r,gm]*I_PD_MS[r,gm,m] for gm∈GM) - PM[r,m]*O_MS_PM[r,m]
        
        prf_C[r=R],	sum(PA[r,g]*I_PA_C[r,g] for g∈G) - PC[r]*O_C_PC[r]
        ...
        mkt_PA[r=R,g=G],		A[r,g]*O_A_PA[r,g] + E_RA_PA[r,g] -( 
                    ...
                    sum(Y[r,s]*I_PA_Y[r,g,s] for s∈S) + I_PA_C[r,g]*C[r])

        #	Producer output supply and demand:

        mkt_PY[r=R,g=G],		sum(Y[r,s]*O_Y_PY[r,g,s] for s∈S) + E_RA_PY[r,g] - X[r,g] * I_PY_X[r,g]

        #	Regional market for goods:

        mkt_PD[r=R,g=G],		X[r,g]*O_X_PD[r,g] - (A[r,g]*I_PD_A[r,g] + sum(MS[r,m]*I_PD_MS[r,g,m] for m∈M))#$gm[g])

        #	National market for goods:

        mkt_PN[g=G],		sum(X[r,g] * O_X_PN[g,r] for r∈R) - (

                    sum(A[r,g] * I_PN_A[g,r] for r∈R) + sum(MS[r,m]*I_PN_MS[g,r,m] for r∈R,m∈M))#$gm[g]

        #	Foreign exchange:

        mkt_PFX,		sum(X[r,g]*O_X_PFX[r,g] for r∈R,g∈G if s0[[r],[g]]!=0) + sum(A[r,g]*O_A_PFX[r,g] for r∈R,g∈G) + sum(E_RA_PFX[r] for r∈R) -

                             sum(A[r,g] * I_PFX_A[r,g] for r∈R,g∈G)
                    
        #	Labor market:

        mkt_PL[r=R],		E_RA_PL[r] - sum(Y[r,s]*I_PL_Y[r,s] for s∈S)

        #	Capital stocks:

        mkt_PK[r=R,s=S],		E_RA_PK[r,s] - Y[r,s]*I_PK_Y[r,s]

        #	Margin supply and demand:

        mkt_PM[r=R,m=M],		MS[r,m]*O_MS_PM[r,m] - sum(A[r,g] * I_PM_A[r,m,g] for g∈G)

        #	Consumer demand:

        mkt_PC[r=R],		C[r]*O_C_PC[r] - D_PC_RA[r]
    ...
    @complementarity(model,prf_Y,Y)
    @complementarity(model,prf_X,X)
    @complementarity(model,prf_A,A)
    @complementarity(model,prf_C,C)
    @complementarity(model,prf_MS,MS)
    @complementarity(model,mkt_PA,PA)
    @complementarity(model,mkt_PY,PY)
    @complementarity(model,mkt_PD,PD)
    @complementarity(model,mkt_PN,PN)
    @complementarity(model,mkt_PL,PL)
    @complementarity(model,mkt_PK,PK)
    @complementarity(model,mkt_PM,PM)
    @complementarity(model,mkt_PC,PC)
    @complementarity(model,mkt_PFX,PFX)
    @complementarity(model,bal_RA,RA)
